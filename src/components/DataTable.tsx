...
export default function DataTable({ data, hoveredEventId, setHoveredEventId, pinnedEventId, setPinnedEventId }: any) {
  ...
  const thClass = "text-right py-[7px] px-2.5 text-[9px] font-bold tracking-[0.1em] uppercase text-[var(--muted)] bg-[var(--surface2)] border-b border-[var(--navy-border)] whitespace-nowrap";
  const tdClass = "text-right py-[7px] px-2.5 whitespace-nowrap";
        ...
        <table className="w-full border-collapse text-[11.5px]">
          <thead>
            <tr>
              ...
              <th className={thClass}>−10D</th>
              <th className={thClass}>−5D</th>
              <th className={thClass}>+5D</th>
              <th className={thClass}>+10D</th>
              <th className={thClass}>+1M</th>
              <th className={thClass}>+3M</th>
          ...
          <tbody>
            ...
            <tr className="bg-[rgba(222,163,89,0.07)] font-bold border-b border-[rgba(31,51,71,0.6)]">
              ...
              <td className={tdClass}>{renderCell(getMedianAtT(aggregate.t_axis, aggregate.median, -10), meta.pre_days < 10)}</td>
              <td className={tdClass}>{renderCell(getMedianAtT(aggregate.t_axis, aggregate.median, -5), meta.pre_days < 5)}</td>
              <td className={tdClass}>{renderCell(getMedianAtT(aggregate.t_axis, aggregate.median, 5), meta.post_days < 5)}</td>
              <td className={tdClass}>{renderCell(getMedianAtT(aggregate.t_axis, aggregate.median, 10), meta.post_days < 10)}</td>
            ...
            {events.map((e: any) => {
              if (!e.window_data) {
                return (
                  <tr key={e.event_id} className="border-b border-[rgba(31,51,71,0.6)] opacity-40">
                    ...
                    {[...Array(8)].map((_, i) => <td key={i} className={tdClass + " text-[var(--muted)]"}>—</td>)}
                  </tr>
                );
              }
              ...
              return (
                ...
                >
                  ...
                  <td className={tdClass}>{renderDataCell(r10,  meta.pre_days < 10,  mm10)}</td>
                  <td className={tdClass}>{renderDataCell(r5,   meta.pre_days < 5,   mm5)}</td>
                  <td className={tdClass}>{renderDataCell(r5p,  meta.post_days < 5,  mp5)}</td>
                  <td className={tdClass}>{renderDataCell(r10p, meta.post_days < 10, mp10)}</td>
                  <td className={tdClass}>{renderDataCell(e.window_data.stats.return_1m,  meta.post_days < 21,  mp1m)}</td>
                  <td className={tdClass}>{renderDataCell(e.window_data.stats.return_3m,  meta.post_days < 63,  mp3m)}</td>
                  <td className={tdClass}>{renderDataCell(e.window_data.stats.return_6m,  meta.post_days < 126, mp6m)}</td>
                  <td className={tdClass}>{renderDataCell(e.window_data.stats.max_drawdown_in_window, false, mdd)}</td>
                </tr>
              );
            })}
          </tbody>
